/**
 * @fileoverview QRCode.js - Standard QR Code generator
 * @license MIT
 */
var QRCode;
(function () {
	...
	QRRSBlock.getRSBlocks = function (typeNumber, errorCorrectLevel) {
		var rsBlock = QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) % QRRSBlock.RS_BLOCK_TABLE.length];
		var count = rsBlock[0];
		var totalCount = Math.floor(rsBlock[1] / count);
		var dataCount = Math.floor(rsBlock[2] / count);
		var list = [];
	...
	QRCode.prototype.makeCode = function (text) {
		...
		var cellSize = this._htOption.width / count;
		...
		if (this._el) {
			...
			img.className = "qr-gen-img";
			this._el.appendChild(img);
		}
		return canvas.toDataURL("image/png");
	};

	QRCode.generateDataUrl = function (text, size, dark, light) {
		try {
			var textLen = (text || "").length;
			var typeNum = textLen > 68 ? 8 : 6;
			var qr = new QRCodeModel(typeNum, QRErrorCorrectLevel.M);
			qr.addData(text || "https://ua-eventos-uy.web.app/coyote-vs-acme?i=UA-DEMO-001");
			qr.make();
			var count = qr.getModuleCount();
			var canvas = document.createElement("canvas");
			...
			console.warn("QRCode.generateDataUrl fallback:", err);
			return "https://api.qrserver.com/v1/create-qr-code/?size=" + (size || 256) + "x" + (size || 256) + "&data=" + encodeURIComponent(text || "") + "&color=0f172a&bgcolor=ffffff";
		}
	};

	QRCode.drawToCanvas = function (ctx, x, y, size, text, dark, light) {
		try {
			var textLen = (text || "").length;
			var typeNum = textLen > 68 ? 8 : 6;
			var qr = new QRCodeModel(typeNum, QRErrorCorrectLevel.M);
			...
			console.warn("QRCode.drawToCanvas fallback:", err);
			ctx.fillStyle = light || "#ffffff";
			ctx.fillRect(x, y, size, size);
		}
	};
